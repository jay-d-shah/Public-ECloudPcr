...
(function($) {
    ...
    jQuery.fn.tweetable = function (options) {
        ...
        return this.each(function (options) {
            ...
            var act = jQuery(this)
            ,   tweetList = jQuery('<ul class="media-list tweetList">')[defaults.position.toLowerCase() + 'To'](act)
            ...
            jQuery.getJSON(api + defaults.username + count + (defaults.limit + 5) + replies + defaults.replies + rts + defaults.retweets + "&callback=?", act, function (data) {
                ...
                if(twitterError)
                {
                    tweetList.append('<li class="media tweet_content"><div class="media-body alert alert-error"><p class="tweet_link">'+ defaults.failed +'</p></div></li>');
                    return;
                }
                ...
                jQuery.each(data, function (i, tweet) {
                    ...
                    tweetList.append('<li class="media tweet_content_' + i + '"><i class="pull-left icon-twitter-sign icon-2x c-primary"></i><div class="media-body"><p class="tweet_link_' + i + '">' + tweet.text.replace(/#(.*?)(\s|$)/g, '<span class="hash">#$1 </span>').replace(/(\b(https?|ftp|file):\/\/[-A-Z0-9+&@#\/%?=~_|!:,.;]*[-A-Z0-9+&@#\/%=~_|])/ig, '<a href="$&">$&</a> ').replace(/@(.*?)(\s|\(|\)|$)/g, '<a href="http://twitter.com/$1">@$1 </a>$2') + '</p></div></li>');
